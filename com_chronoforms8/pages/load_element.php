...
$listBehaviors = function($id, $list, $default = []){
	$options = [];
	...
	foreach($list as $behavior){
		...
		$options[] =  new Option(text:$info->text, value:$behavior, html:'<span class="nui label '.$color.'">'.(isset($info->icon) ? Chrono::ShowIcon($info->icon) : "").$info->text.'</span><span class="nui right">'.$info->description."</span>", selected:in_array($behavior, $default));
	}
...
if(!file_exists($info_path)){
	ChronoSession::setFlash("error", "View json file not found:".$info_path);
	return;
}
...
if(!empty($element["type"]) && $element["type"] == "actions"){
	$label .= '&nbsp;<span class="nui label rounded grey">'.CF8::getname($element).'</span>';
}else if($type == "actions"){
	$label .= '&nbsp;<span class="nui label rounded grey">'.$name.$id.'</span>';
}
if(!empty($element["label"])){
	$label .= '&nbsp;<span class="nui label rounded grey">'.(strlen($element["label"]) > 50 ? substr($element["label"], 0, 50)."..." : $element["label"]).'</span>';
}
...
<div data-pid='<?php echo $id; ?>' data-title='<?php echo $info->title; ?>' class='nui p1 bordered white rounded draggable form_item dropped'>
	...
	<div class="nui hidden actions source">
		<div class="nui label blue rounded link edit_item"><?php echo Chrono::ShowIcon("wrench"); ?></div>
		<div class="nui label yellow rounded link drag_item"><?php echo Chrono::ShowIcon("sort"); ?></div>
		<div class="nui label red rounded link remove_item"><?php echo Chrono::ShowIcon("xmark"); ?></div>
	</div>
	...
	<?php 
	$areas = "";
	if(empty($info->areas) && !empty($element["events"])){
		$info->areas = [];
