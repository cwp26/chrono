...
<?php foreach(["n" => []] + (!empty($this->data["elements"][$id]["triggers"]) ? $this->data["elements"][$id]["triggers"] : []) as $k => $item): ?>
	<div class="nui form clonable triggers-<?php echo $id; ?>" data-selector=".clonable.triggers-<?php echo $id; ?>" data-cloner=".triggers-<?php echo $id; ?>-cloner" data-key="<?php echo $k; ?>">
		<div class="equal fields">
			<?php
				new FormField(name: "elements[$id][triggers][".$k."][condition]", type:"select", label: "On", options:[
				...
				new FormField(name: "elements[$id][triggers][".$k."][value]", type:"select", label: "Value(s) - Optional", multiple:true, code:"data-additions='1'");
				// new FormField(name: "elements[$id][triggers][".$k."][name]", label: "Trigger");
				new FormField(name: "elements[$id][triggers][".$k."][name]", type:"select", multiple:true, label: "Trigger(s)", code:" data-additions='1'", options:[]);
			?>
